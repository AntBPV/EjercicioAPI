...
class DivisionView(APIView):
    def get(self,request,*args,**kwargs):
        ...
        try:
            ...
            if num_dos == 0:
                return Response("No se puede dividir por cero", status=status.HTTP_400_BAD_REQUEST)

            division = num_uno / num_dos
            return Response(division, status=status.HTTP_200_OK)

        except ValueError:
            return Response("Los números deben ser enteros válidos", status=status.HTTP_400_BAD_REQUEST)
